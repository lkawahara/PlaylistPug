...
public class MainConfig 
{
    ...
    dbUrl = "jdbc:postgresql://" +
			...
			"?user=" + username +
			"&password=" + password;
    /*Above: broken need dbName*/
    
    
    /*
    //Standard Heroku Postgres diver
    private String standardHerokuPostgresDiverSetup(URI dbUri)
    {
    	String username = dbUri.getUserInfo().split(":")[0];
        String password = dbUri.getUserInfo().split(":")[1];
		return "postgres://" + username + ':' + password + '@' + dbUri.getHost() + '/' + dbUri.getPath();
    }
    
    //JDBC Heroku Postgres driver
    private String JDBCHerokuPostgresDiverSetup(URI dbUri)
    {
    	String username = dbUri.getUserInfo().split(":")[0];
        String password = dbUri.getUserInfo().split(":")[1];
		String dbUrl = "jdbc:postgresql://" + dbUri.getHost() + ':' + dbUri.getPort()  +
				'/' + dbUri. +"?user=" + username + dbUri.getPath();
